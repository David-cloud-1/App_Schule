...
// Correctness is decided server-side from the answer key (PROJ-19 BUG-2).
// By default the "key" agrees with each fixture's is_correct flag; one test
// overrides it to prove a client's own claim is ignored.
...
// The profile write now goes through the service client; tests route it to
// the same mock so the existing SELECT-then-UPDATE call order still holds.
...
describe('POST /api/quiz/sessions', () => {
  ...
  it('ignores the client-reported is_correct and scores from the server-side key', async () => {
    ...
    expect(body.coins_earned).toBe(0)
  })
